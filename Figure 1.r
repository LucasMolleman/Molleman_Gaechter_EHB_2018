...
for (aspect in 1:2)
{
	...
	for (cit in unique(a$city)){
		...
		else lines(c(0,0:200/200), c(0,cumFreq), col=adjustcolor('#FF9C00', alpha=1), lwd=1, lty=ltys[cityCnt])
	...
	for (countr in c('China', 'UK')){
		...
		if (aspect==1) srt<-sort(b$SDreq)
		if (aspect==2) srt<-sort(b$SDpayInfo)
		...
		else lines(c(0,0:200/200), c(0,cumFreq), col=adjustcolor('#FF9C00', alpha=0.5), lwd=3, lty=1)
	}
}

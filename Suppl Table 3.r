...
#treatment 2 is the Social dilemma
...
#Model 1
m1<-glmer(f ~ country + period + prevPay + (1|uniqueSes/uniquePlayerNr), family='binomial', weights=n, data=data)
summary(m1)

#Model 2
m2<-glmer(f ~ country + period + prevDec + (1|uniqueSes/uniquePlayerNr), family='binomial', weights=n, data=data)
summary(m2)


#Model 3
m3<-glmer(f ~ city + period + prevPay + (1|uniqueSes/uniquePlayerNr), family='binomial', weights=n, data=data)
summary(m3)

#Model 4
m4<-glmer(f ~ city + period + prevPay + (1|uniqueSes/uniquePlayerNr), family='binomial', weights=n, data=subset(data, data$country=='China'))
summary(m4)

#Model 5
m5<-glmer(f ~ city + period + prevPay + (1|uniqueSes/uniquePlayerNr), family='binomial', weights=n, data=subset(data, data$country=='UK'))
summary(m5)


#Additional model (only period 2; no feedback in this block yet)
m1a<-glmer(f ~ country + prevPay + (1|uniqueSes/uniquePlayerNr), family='binomial', weights=n, data=subset(data, data$period==2))
summary(m1a)
